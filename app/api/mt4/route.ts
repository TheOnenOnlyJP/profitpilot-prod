...
const login = process.env.MT4_LOGIN || "<put in your MT login here>";
const password = process.env.MT4_PASSWORD || "<put in your MT password here>";
const serverName =
  process.env.MT4_SERVER || "<put in your MT server name here>";
...
async function testMetaApiSynchronization() {
  try {
    ...
    let account = accounts.find(
      (a: any) => a.login === login && a.type.startsWith("cloud")
    );
    if (!account) {
    ...
    console.log(
      "Undeploying MT4 account so that it does not consume any unwanted resources"
    );
